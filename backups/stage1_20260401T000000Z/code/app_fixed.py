...
class VisualizationEngine:
    ...
    def _load_smallnet_series(self, smallnet_path: str, compare_path: str):
        ...
        if os.path.exists(smallnet_path):
            ...
            for tc in small_raw:
                for m in tc.get("methods", []):
                    small_stats[m.get("method", "")].append(float(m.get("parse_rate", 0) or 0))
        ...
        if os.path.exists(compare_path):
            ...
            for tc in large_raw:
                for m in tc.get("methods", []):
                    val = m.get("constraint_rate", None)
                    if val is None:
                        val = m.get("parsed_edges", 0)
                    large_stats[m.get("method", "")].append(float(val or 0))
        ...
        method_order = ["Qwen-LoRA★", "R1-LoRA★", "Sparse-LoRA★★"]
    ...
    def plot_smallnet_decay_compare(self, smallnet_path: str, compare_path: str):
        ...
        fig.update_layout(
            title=dict(text="SmallNet vs LargeNet 解析率衰减对比", x=0.5, font=dict(size=16)),
            xaxis=dict(title="模型方法", tickangle=-12),
            yaxis=dict(title="约束符合率(%)", range=[0, 110], gridcolor="#E5E7EB"),
            barmode="group",
...
""")
            ...
            若小路网解析率接近 100%，而大路网（98条边）解析率明显下降，
            ...
            with right_col:
                ...
                if primary:
                    ...
                    <b>{primary.get('label', primary.get('method', '全量模型'))}</b> 的解析率下降了
            ...
            if _os2.path.exists(SMALLNET_RESULT_PATH):
                ...
                st.markdown("""<div class="info-strip">
                ✅ <b>结论</b>：SmallNet（24条边）与 LargeNet（98条边）并排对照后，
                可以直接看到全量输出方法在大路网上的解析率衰减，而本文方法保持更稳定的约束符合率。
                </div>""", unsafe_allow_html=True)
            else:
