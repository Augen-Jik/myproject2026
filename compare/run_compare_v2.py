...
def run_all(run_llm=True):
    ...
    if run_llm:
        ...
        if _os.path.exists(_sp):
            print("📦 加载 Qwen-Sparse★（稀疏输出+GAT补全，本文方法）...")
            sparse_tok, sparse_mdl = load_llm(_sp)
        else:
            print(f"⚪ 稀疏模型待训练: {_sp}")
        import os as _os
        if _os.path.exists("/root/autodl-tmp/DeepSeek-R1-1.5B"):
            print("📦 加载 R1-Raw（DeepSeek-R1基座）...")
            r1_raw_tok, r1_raw_mdl = load_llm("/root/autodl-tmp/DeepSeek-R1-1.5B")
        if _os.path.exists("/root/autodl-tmp/model_merged_r1"):
            print("📦 加载 R1-SFT（R1微调模型）...")
            r1_sft_tok, r1_sft_mdl = load_llm("/root/autodl-tmp/model_merged_r1")
        else:
            print("⚠️  R1-SFT 尚未训练完成，跳过")
    ...
    for tc in TEST_CASES:
        ...
        if run_llm:
            ...
            def _sparse():
                ...
                import torch as _t, copy
                _instr = tc["desc"]
                if not any(k in _instr for k in ["异常","输出","分配","路段权重"]):
                    _instr = _instr.rstrip("。") + "。请输出路况异常的路段权重（偏离正常值2.0超过1.0的路段）。"
                _prompt = sparse_tok.apply_chat_template(
                    [{"role":"user","content":_instr}],
                    tokenize=False, add_generation_prompt=True)
                _inputs = sparse_tok(_prompt, return_tensors="pt",
                                     truncation=True, max_length=1024).to(sparse_mdl.device)
                _in_len = _inputs["input_ids"].shape[1]
                with _t.no_grad():
                    _out = sparse_mdl.generate(**_inputs, max_new_tokens=400,
                                               do_sample=False, repetition_penalty=1.0,
            ...
            def _sparse_gat():
                if sparse_tok is None or sparse_mdl is None:
                    return {e:1.0 for e in ALL_EDGES}, [], float("inf")
                import torch as _t, copy, sys as _sys
                _cd = "/root/autodl-tmp/code"
                if _cd not in _sys.path: _sys.path.insert(0, _cd)
                _instr = tc["desc"]
                if not any(k in _instr for k in ["异常","输出","分配","路段权重"]):
                ...
                if "</think>" in raw: raw = raw.split("</think>")[-1]
                w = parse_weights(raw)
                for e in ALL_EDGES: w.setdefault(e, 2.0)
                try:
                    from gat_smoother import smooth_weights
                    w = smooth_weights(copy.deepcopy(w),
                                       "/root/autodl-tmp/gat_model.pt", device="cpu")
                except Exception as _ge: print(f"     [GAT跳过] {_ge}")
                p, c = astar_route(net, tc["start"], tc["end"], w)
                return w, p, c
            methods_to_run += [
                ("Raw-Qwen（0-shot）", _raw),
                ("CoT-Qwen（提示工程）", _cot),
                ("Qwen-LoRA★（本文）", _sft),
                ("Qwen-LoRA+GAT★★",   _lora_gat),
                ("Qwen-Sparse★", _sparse),
                ("Sparse+GAT★★", _sparse_gat),
                ("R1-Raw",    _r1_raw),
                ("R1-SFT★",   _r1_sft),
            ]
