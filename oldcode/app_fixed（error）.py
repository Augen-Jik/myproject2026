...
class VisualizationEngine:
    ...
    def plot_compare_plotly(self, your_time, your_cost):
        ...
        # 用 json 往返彻底剥离 numpy 类型
        import json as _json
        _t = _json.loads(_json.dumps([float(v) for v in times]))
        _r = _json.loads(_json.dumps([int(v)   for v in rates]))
        _c = _json.loads(_json.dumps([float(v) for v in costs]))
        df = pd.DataFrame({
            "Method":              list(methods),
            "Travel Time (s)":     _t,
            "Constraint Rate (%)": _r,
...
def main():
    ...
    try:
        ...
        with compare_ph.container():
            cmp_fig, cmp_df = viz_engine.plot_compare_plotly(float(f"{travel_time:.6f}"), float(f"{total_cost:.6f}"))
