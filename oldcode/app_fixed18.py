...
class PathPlanningEngine:
    ...
    def _infer_qwen(_self, tokenizer, model, constraint, device):
        ...
        with torch.no_grad():
            out = model.generate(
                **inputs, max_new_tokens=2500, do_sample=False,
    ...
    def _infer_lora(_self, tokenizer, model, constraint, device):
        ...
        max_new_tokens=2500 保证 <think> + 98条权重能完整输出
        """
        ...
        with torch.no_grad():
            out = model.generate(
                **inputs,
                max_new_tokens=2500,      # 足够容纳 <think> + 98条权重
                do_sample=False,
    ...
    def generate_weights(_self, constraint, model_name, _model_manager):
        ...
        - Qwen → _infer_qwen
        - R1   → _infer_r1
        - fallback: 若LLM解析<10条，用规则解析兜底（保证A*有合理权重）
        """
        try:
            tokenizer, model = _model_manager.get_model(model_name)
            device = _self.config.get("DEVICE", "cpu")
            # 精确识别模型类型
            _is_lora    = "LoRA" in model_name or "lora" in model_name
            ...
            logging.info(f"Weights {infer_time:.2f}s | llm_parsed={llm_parsed_count} | is_r1_sft={_is_r1_sft} | is_r1_raw={_is_r1_raw}")
            # 返回5个值：struct, weight_dict, infer_time, raw, llm_parsed_count
            return struct, wd, infer_time, raw, llm_parsed_count
...
def main():
    ...
    with st.sidebar:
        ...
        selected_model = st.selectbox(
            "🤖 权重生成模型",
            [
                "Qwen2.5-1.5B-LoRA (本文方法)",     # 👈 必须包含 "LoRA" 关键字，触发新推理方法
                "DeepSeek-R1-1.5B-LoRA (推理增强)", # 👈 预留给一会要合并的 R1
        ...
        # 模型路径映射
        _model_path_map = {
            "Qwen2.5-1.5B-LoRA (本文方法)":       "/root/autodl-tmp/model_merged_qwen",  # 👈 指向你刚才合并好的正确路径
            "DeepSeek-R1-1.5B-LoRA (推理增强)":   "/root/autodl-tmp/model_merged_r1",    # 👈 R1的新路径预留
        ...
        scenarios = {
            "测试1 黄河路单向拥堵": (
                "R3C0_E", "R3C4_E",
                "黄河路经一路至经三路段向东因追尾事故严重拥堵；黄河路经三路至经六路段向东中度拥堵；"
                "花园路向北畅通无阻。请为全部98条路段生成权重。"),
            "测试2 花园路封闭绕行": (
                "C4R0_N", "C4R3_N",
                "花园路农业路至红专路段向北施工封闭；花园路红专路至政七街段向北临时管制；"
                "经六路向北畅通无阻。请为全部98条路段生成权重。"),
            "测试4 中央节点封锁": (
                "R0C0_E", "R4C3_E",   # 原R4C4_W改为R4C3_E（纬五路经八路段，避免netconvert删边）
                "经六路农业路至黄河路段全线封闭施工；花园路政七街至黄河路段因重大活动管制；"
                "经三路向南追尾事故极度拥堵；经八路向北正常通行。请为全部98条路段生成权重。"),
            "测试6 突发大规模事故": (
                "R0C0_E", "C4R3_N",
                "突发大规模事故：花园路农业路至黄河路全段封闭；经六路农业路至红专路段三车连环追尾完全封闭；"
                "黄河路向东中度拥堵；经三路向北畅通作为绕行选择。请为全部98条路段生成权重。"),
        }
    ...
    try:
        ...
        if st.session_state.get("use_manual") and manual_str.strip():
            ...
            struct_output, weight_dict, infer_time, raw_output, _llm_parsed = path_engine.generate_weights(
            ...
            if _llm_parsed < 10:
                st.warning(f"⚠️ 模型仅解析 {_llm_parsed}/98 条，已用规则解析兜底保证路径规划正常运行")
            else:
                st.info(f"ℹ️ 模型解析 {_llm_parsed}/98 条语义权重，用于A*路径决策")
        ...
        with weight_ph.container():
            ...
            kc4.metric("📊 解析路段", f"{st.session_state.get('llm_parsed', len(weight_dict))} / 98")
            # ── 关键路段对比（模型权重 vs 规则真实值）────────────────
            _scene_ref = path_engine._rule_parse_weights(traffic_constraint)
        ...
        with metric_ph.container():
            ...
            if speed_data:
                ...
                with g_col:
                    ...
                    _scene_w_gantt = path_engine._rule_parse_weights(traffic_constraint)
