...
class PathPlanningEngine:
    ...
    def _infer_r1(_self, tokenizer, model, constraint, device):
        """R1专用推理：apply_chat_template + 三重提取策略"""
        # 使用完整边ID列表，防止模型幻觉出中文路名
        r1_instr = (
            "你是交通路径权重生成助手。请根据交通描述，为郑州金水区路网全部98条路段分配拥堵权重。\n"
            "【输出要求】\n"
            "- </think>标签结束后，直接输出权重列表，不要有其他文字\n"
            "- 格式：路段ID:数值, 路段ID:数值（每行或逗号分隔均可）\n"
            "- 权重范围0-10，映射：封闭=9.5, 严重拥堵=7.5, 中度拥堵=5.0, 正常=2.0, 畅通=1.2\n"
            "- 必须只使用以下98个路段ID，禁止使用中文路名：\n"
    ...
    def generate_weights(_self, constraint, model_name, _model_manager):
        ...
        try:
            ...
            if _is_r1:
                raw, struct, infer_time = _self._infer_r1(tokenizer, model, constraint, device)
            else:
            ...
            logging.info(f"Weights {infer_time:.2f}s | llm_parsed={llm_parsed_count} | R1={_is_r1}")
