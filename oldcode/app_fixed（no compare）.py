...
class SUMOSimulationEngine:
    ...
    def run_simulation(_self, path_tuple):
        ...
        # 清空预定义路线，避免 routes.rou.xml 中无效路线导致 SUMO 退出
        try:
            rou_path = "/root/autodl-tmp/SUMO/routes.rou.xml"
            with open(rou_path, "w") as _rf:
                _rf.write("<routes></routes>\n")
        except Exception:
            pass
        ...
        try:
            ...
            # 强制转为纯 Python 类型，防止 numpy 类型污染后续代码
            travel_time = float(travel_time)
            avg_speed   = float(avg_speed)
            speed_data  = [float(v) for v in speed_data]
            pos_data    = [(float(p[0]), float(p[1])) for p in pos_data]
...
class VisualizationEngine:
    ...
    def plot_speed_plotly(self, speed_data, step_len=0.1):
        # 用 go.Scatter 替代 px.line，绕开 pandas Series 类型转换问题
        spd = [float(v) for v in speed_data]
        t   = [float(i * step_len) for i in range(len(spd))]
        avg_spd = float(sum(spd) / len(spd)) if spd else 0.0
        fig = go.Figure()
        fig.add_trace(go.Scatter(
    ...
    def plot_compare_plotly(self, your_time, your_cost):
        ...
        df = pd.DataFrame.from_records([
            {"Method": str(methods[i]),
             "Travel Time (s)":     float(times[i]),
             "Constraint Rate (%)": int(rates[i]),
             "Path Cost":           float(costs[i])}
...
def main():
    ...
    sub = f"Open-R1 SFT · SUMO · A* · 6×5 郑州金水区路网 · 规划次数: {run_cnt}"
    if best_t > 0:
    ...
    with st.sidebar:
        ...
        st.markdown('<div class="sec-hdr">🔍 模型原始输出</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        with compare_ph.container():
            ...
            radar_fig2 = go.Figure(go.Scatterpolar(
                ...
                line=dict(color="#3B82F6", width=2), name="Constraint Rate",
            ))
            radar_fig2.update_layout(
                polar=dict(radialaxis=dict(range=[0, 110], tickvals=[25, 50, 75, 100])),
                height=340, paper_bgcolor="white", margin=dict(l=60, r=60, t=40, b=40),
            )
