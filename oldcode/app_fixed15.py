...
class ModelManager:
    ...
    def load_model(_self, model_name, model_path):
        ...
        # transformers>=4.45 将 load_in_8bit/load_in_4bit 移入 BitsAndBytesConfig
        # 用 try/except 兼容新旧版本，1.5B模型不需要量化
        try:
            model = AutoModelForCausalLM.from_pretrained(model_path, **_load_kwargs)
        except TypeError:
            _load_kwargs.pop("load_in_8bit", None)
            _load_kwargs.pop("load_in_4bit", None)
            model = AutoModelForCausalLM.from_pretrained(model_path, **_load_kwargs)
        model.eval()
...
class PathPlanningEngine:
    ...
    def generate_weights(_self, constraint, model_name, _model_manager):
        ...
        try:
            ...
            if _is_r1:
                # ── R1专用推理路径 ──────────────────────────────────
                # 1. prompt包含完整边ID列表，防止幻觉出中文路名
                # 2. add_generation_prompt=True 与训练格式对齐
                # 3. repetition_penalty=1.3（1.5会截断98条重复格式）
                # 4. 剥离<think>...</think>推理链，只取结构化输出
                _r1_instr = (
                ...
                with torch.no_grad():
                    out = model.generate(
                        **inputs, max_new_tokens=1200, do_sample=False,
                        ...
                        eos_token_id=tokenizer.eos_token_id,
                        pad_token_id=tokenizer.pad_token_id or tokenizer.eos_token_id,
                        repetition_penalty=1.5,  # Qwen-SFT原始设置，保持97/98
                    )
                infer_time = time.time() - t0
    ...
    try:
        ...
        if st.session_state.get("use_manual") and manual_str.strip():
            ...
            if not weight_dict:
                ...
                prog.empty(); status.empty(); return
            # 未解析路段补中性默认值(2.0=正常通行)，统一仿真基准，保留真实解析数量显示
            _ALL_EDGE_IDS = (
        ...
        # 仿真使用当前模型权重（已在上方补齐98条默认值），各模型基准一致，结果可横向对比
