...
class ModelManager:
    ...
    @st.cache_resource(ttl=3600, show_spinner="正在加载大模型...")
    def load_model(_self, model_name, model_path):
        ...
        # R1/DeepSeek 必须 left padding；Qwen 用 right
        tokenizer.padding_side = "left" if ("R1" in model_path or "DeepSeek" in model_path) else "right"
...
class PathPlanningEngine:
    ...
    def generate_weights(_self, constraint, model_name, _model_manager):
        # ── 全部98条合法路段ID（来自 build_urban_net.py）──────────────
        _ALL_EDGES = (
        ...
        try:
            ...
            if _is_r1:
                # R1推理：user内容与训练数据格式一致（含完整路网说明+边ID列表）
            ...
            if _is_r1:
                ...
                # ── 关键修复：剥离 <think>...</think> 推理链，只取其后的结构化输出 ──
                if "</think>" in raw:
...
def main():
    ...
    try:
        ...
        if st.session_state.get("use_manual") and manual_str.strip():
            ...
            # Qwen-SFT 权重覆盖率高时缓存为限速基准
            if "Qwen" in selected_model and "SFT" in selected_model and len(weight_dict) >= 80:
                st.session_state["qwen_sft_weights"] = weight_dict
                st.session_state["qwen_sft_desc"]    = traffic_constraint
            if not weight_dict:
                st.error("❌ 模型未生成有效权重")
                ...
                prog.empty(); status.empty(); return
            # 解析不足50条时，用 Qwen-SFT 缓存权重补全未覆盖路段
            _cached_qwen = st.session_state.get("qwen_sft_weights", {})
            if len(weight_dict) < 50 and _cached_qwen:
                _filled = {**_cached_qwen, **weight_dict}  # 当前模型权重优先
                st.warning(f"⚠️ {selected_model.split('(')[0].strip()} 解析 {len(weight_dict)}/98 条，"
                           f"已用 Qwen-SFT 权重补全至 {len(_filled)} 条")
                weight_dict = _filled
        ...
        # 限速基准：优先用 Qwen-SFT 权重（覆盖率最高），其次用当前模型权重
        # 这样无论选什么模型，SUMO 仿真都基于同一套真实路况
        _qwen_wd = st.session_state.get("qwen_sft_weights", {})
        _sim_weights = _qwen_wd if len(_qwen_wd) >= 80 else weight_dict
        if _qwen_wd and len(_qwen_wd) >= 80 and "SFT" not in selected_model.split("(")[0]:
            st.info(f"ℹ️ SUMO限速基准：使用 Qwen-SFT 权重（{len(_qwen_wd)}条），"
                    f"确保与路况一致；当前模型 {selected_model.split('(')[0].strip()} "
                    f"权重用于路径规划（{len(weight_dict)}条）")
        travel_time, avg_speed, speed_data, pos_data = sumo_engine.run_simulation(
            tuple(path), tuple(sorted(_sim_weights.items())))
