...
class PathPlanningEngine:
    ...
    def generate_weights(_self, constraint, model_name, _model_manager):
        ...
        try:
            ...
            if len(wd) < 10:
                logging.warning(f"LLM仅解析{len(wd)}条，启用规则解析兜底")
                wd = _self._rule_parse_weights(constraint)
                raw = (raw or "") + "\n[⚠️ 模型输出不足，已用规则解析兜底]"

            logging.info(f"Weights {infer_time:.2f}s | parsed={len(wd)} | R1={_is_r1}")
    ...
    def astar_route(self, net, start_eid, end_eid, weight_dict):
        ...
        # 简单A*启发函数：欧几里得距离/最大速度（米/权重单位）
        def _heuristic(node):
            try:
                ex, ey = ee.getToNode().getCoord()
                nx, ny = node.getCoord()
                return math.sqrt((ex-nx)**2 + (ey-ny)**2) / 600.0
            except Exception:
                return 0.0

        open_list = {sn: (weight_dict.get(start_eid, 2.0) + _heuristic(sn), 0.0, [start_eid])}
        closed    = set()
        while open_list:
    ...
    try:
        ...
        if st.session_state.get("use_manual") and manual_str.strip():
            ...
            if not weight_dict:
                ...
                prog.empty(); status.empty(); return
            _parsed_cnt = len([v for v in weight_dict.values() if v != 2.0])
            st.info(f"ℹ️ 模型解析 {_parsed_cnt}/98 条语义权重，用于A*路径决策")
