...
面向自然语言动态交通约束的稀疏语义解析与路径规划系统

本项目对应论文《面向自然语言动态交通约束的稀疏语义解析与路径规划方法研究》，提出以**结构化时空增强输入 + 稀疏语义锚点输出 + 经典搜索 + SUMO 闭环验证**为主线的混合式框架，在 96 条有向边的实际路网上与 Dijkstra、Rule-A\*、DQN、GCN-Weight 等基线方法进行完成状态感知的闭环对比。
...
本项目关注的不是静态最短路，而是“道路状态正在变化时，如何把人用自然语言说出的交通信息转成可计算的路径规划约束”。下面的例子都对应 `code/scenarios.py` 中的应用场景预设，可以理解为系统在真实交通管理中的使用方式。

**例子 1：前方事故，需要临时绕行**

- 用户输入：`黄河路经一路至经三路段向东因追尾事故严重拥堵，花园路向北畅通。`
- 系统识别：黄河路、向东、经一路到经三路、严重拥堵；花园路向北保持正常。
- 规划效果：路径会尽量避开黄河路向东的事故路段，但不会误伤黄河路反方向或花园路向北道路。
- 应用意义：交警或调度员只需输入一句事故描述，系统就能把它转成路网约束，减少手动改边权的工作。

**例子 2：单向施工封闭，另一方向仍可通行**

- 用户输入：`花园路农业路至红专路段向北施工封闭，经六路向北畅通无阻。`
- 系统识别：花园路指定区间的北向道路不可通行；经六路北向不受影响。
- 规划效果：车辆不会被规划到封闭的北向施工段，但仍可使用其他正常方向和道路绕行。
- 应用意义：适合临时施工、单向管制、道路半幅封闭等场景，重点是区分“哪条路、哪一段、哪个方向”。

**例子 3：大型活动散场，局部道路限行**

- 用户输入：`演唱会散场临时管制：纬五路向东封闭，向西保持畅通；未来路向南因接驳车排队中度拥堵。`
- 系统识别：纬五路东向封闭、西向正常；未来路南向中度拥堵。
- 规划效果：系统优先避开纬五路东向封闭段，并对未来路南向拥堵段增加通行代价。
- 应用意义：可用于演唱会、球赛、展会等活动后的车辆疏散和接驳调度，把现场口头通知快速变成可计算方案。

**例子 4：多处同时出问题，需要整体评估**

- 用户输入：`经六路全线封闭施工，花园路向北严重拥堵，黄河路向东中度拥堵。`
- 系统识别：经六路全线封闭；花园路北向严重拥堵；黄河路东向中度拥堵。
- 规划效果：路径搜索会同时考虑三类约束，并可接入 SUMO 仿真比较不同绕行方案的行程时间和约束遵守率。
- 应用意义：适合事故、施工、拥堵外溢同时发生的应急场景，帮助判断哪条绕行方案更稳妥。

这些例子展示了本系统的核心作用：让交通管理人员或导航系统可以直接输入自然语言事件描述，由模型抽取少量关键异常路段，再交给经典路径搜索和 SUMO 仿真验证。相比让大模型一次性生成全路网边权，稀疏锚点方式更短、更稳定，也更容易解释每一次路径变化来自哪条道路、哪个方向、哪类事件。
