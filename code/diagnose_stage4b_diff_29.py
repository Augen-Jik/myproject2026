...
DEFAULT_PRE_MODEL = "/root/autodl-tmp/model_merged_sparse_v2"
DEFAULT_POST_MODEL = "/root/autodl-tmp/model_merged_sparse_v2_stage4b_fix"
...
DEFAULT_OUTPUT_JSON = "/root/autodl-tmp/results/stage4b_diff_29.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Diff diagnose pre vs stage4b on 29 held-out main failure samples.")
...
def load_dataset_rows(dataset_root: Path, target_samples: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_df = dataset_df[dataset_df["sample_id"].astype(str).isin(keep_ids)].copy()
    return dataset_df
    ...
    *,
    ...
    stats = {
        ...
        "post_anchor_and_fully_correct_count": sum(
            1
            for item in samples
            if item["post_output"] == "ANCHOR" and not item["post_anchor_still_wrong"]
        ),
        "subtype_breakdown": (
...
def print_summary(report: dict[str, Any]) -> None:
    stats = report["stats"]
    print("STAGE4B_DIFF_29_SUMMARY")
    print(f"sample_count={stats['sample_count']}")
    print(f"completely_unchanged_count={stats['completely_unchanged_count']}")
    print(f"no_anchor_to_anchor_count={stats['no_anchor_to_anchor_count']}")
    print(f"anchor_but_still_wrong_count={stats['anchor_but_still_wrong_count']}")
    print(f"anchor_to_no_anchor_count={stats['anchor_to_no_anchor_count']}")
    print(f"anchor_changed_count={stats['anchor_changed_count']}")
    print(f"majority_answer={stats['majority_answer']}")
